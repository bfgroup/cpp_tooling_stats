...
answer the important question of wether that goal was achieved by current
...
compile individual all source files in parallel executions of the
...
{CPP} construct fail to compile reliably, for example templates, and cause ICEs.
...
++++
<div id="parallel_build" style="height: 500px;"></div>
<script type="text/javascript">
var parallel_build_chart = echarts.init(document.getElementById('parallel_build'));
parallel_build_chart.setOption({
    legend: {
        data:['Non-Modular', 'Modular']
    },
    tooltip: {
        trigger: 'axis',
        axisPointer: {
    ...
    xAxis: [ { name: 'DAG Depth', type: 'category' } ],
    yAxis: [ { name: 'Wall Clock Seconds', type: 'value' } ],
    series: [
        {
            name: 'Non-Modular',
            type: 'bar',
            label: { normal: { show: true, position: 'top', rotate: 90, align: 'left', verticalAlign: 'middle' } },
...
This also indicates would indicate that the larger the number of real
parallel builds possible the less advantage modular compiles have over
non-modular compiles. This would be something to investigate further with
more capable hardware.

== Conclusion

With the limitations of the sampling currently possible the best we can
conclude is that there are some possible gains in modular compilation
but that the gains might not be attainable for really large and complex
...
decipher how to get the GCC svn checkout build in OSX.
