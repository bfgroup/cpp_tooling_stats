...
certainly looks really good for a modular future.

Now, lets look at Clang, the dashed lines, like GCC it starts out
...
Even with the bad news of Clang at high DAG depths this looks promising.
...
than the Clang non-modular builds.
...
builds don't substantially change from the 8 parallel job case. 
...
We've now reached the point where non-modular builds are faster at all DAG
